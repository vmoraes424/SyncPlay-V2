...
pub fn load_mixer_routing() -> MixerRouting {
    std::fs::read_to_string(MIXER_CONFIG_PATH)
        .ok()
...
/// Calcula o volume efetivo de um canal no bus master.
/// Retorna 0.0 se o canal estiver mutado ou não roteado para o master.
pub fn effective_channel_gain(
    ...
    let bus_gain = if routing.master.muted {
        0.0
    } else {
        routing.master.gain
    };

    fade_factor * ch.value * bus_gain
}
